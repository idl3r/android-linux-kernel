				...
				unsigned int code, int value)
{
	...
	if ((bdata->dis_while_sliding) && (code == SW_LID)) {
		...
		struct synaptics_rmi4_data *rmi4_data, bool set)
{
	...
	dev_dbg(rmi4_data->pdev->dev.parent,
		"%s inadvertent tap timer\n", (set != 0) ? "Start" : "Stop");
	if (set != 0) {
		timeout_ms = rmi4_data->mtouch_counter.inadv_tap_det_tmout_ms;
		mod_timer(&rmi4_data->mtouch_counter.inadv_tapdet_timer,
					jiffies + msecs_to_jiffies(timeout_ms));
	} else {
		del_timer_sync(&rmi4_data->mtouch_counter.inadv_tapdet_timer);
	}
}
				...
				struct synaptics_rmi4_data *rmi4_data)
{
	...
	dev_dbg(rmi4_data->pdev->dev.parent,
			"Restart inadvertent tap timer\n");
	timeout_ms = rmi4_data->mtouch_counter.inadv_tap_det_tmout_ms;
	mod_timer(&rmi4_data->mtouch_counter.inadv_tapdet_timer,
				jiffies + msecs_to_jiffies(timeout_ms));
}

static void synaptics_rmi4_mtouch_inadv_detect_timer_handler(
				unsigned long arg)
{
	struct synaptics_rmi4_data  *rmi4_data =
		(struct synaptics_rmi4_data *) arg;
				...
				struct work_struct *work)
{
	...
	dev_dbg(rmi4_data->pdev->dev.parent,
		"%s: Inadvertent tap detection timer expired \n", __func__);
				...
				struct synaptics_rmi4_data *rmi4_data)
{
	...
	init_timer(&rmi4_data->mtouch_counter.inadv_tapdet_timer);
	rmi4_data->mtouch_counter.inadv_tapdet_timer.data = (unsigned long)rmi4_data;
	rmi4_data->mtouch_counter.inadv_tapdet_timer.function =
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	synaptics_dsx_ddt_send(rmi4_data, input);
		...
		struct device_attribute *attr, char *buf)
{
	...
	size += snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf),
			"Failed taps in proxi = %d\n",
			mtouch_counter->failed_taps_in_proxi);
	size += snprintf(buf + strlen(buf), PAGE_SIZE - strlen(buf),
			"Inadvertent tap failures = %d\n",
		...
		case 0x03:
			...
			if (rmi4_data->smart_flip_state)
				...
				case -EAGAIN:
					mtouch_counter->proxi_not_ready++;
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_DDT
					synaptics_dsx_ddt_send(rmi4_data, MTOUCH_PROXI_NOT_READY);
#endif /*CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_DDT*/
					break;
				default:
					break;
