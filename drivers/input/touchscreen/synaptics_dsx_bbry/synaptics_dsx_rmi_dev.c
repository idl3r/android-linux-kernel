...
struct rmidev_attribute {
	struct attribute	attr;
	ssize_t (*show)(struct rmidev_handle *rmidev,
			struct rmidev_attribute *attr, char *buf);
	ssize_t (*store)(struct rmidev_handle *rmidev,
			struct rmidev_attribute *attr, const char *buf,
			size_t count);
};
...
static ssize_t rmidev_sysfs_open_store(struct rmidev_handle *rmidev,
		struct rmidev_attribute *attr, const char *buf, size_t count);

static ssize_t rmidev_sysfs_release_store(struct rmidev_handle *rmidev,
		struct rmidev_attribute *attr, const char *buf, size_t count);

static ssize_t rmidev_sysfs_attn_state_show(struct rmidev_handle *rmidev,
		struct rmidev_attribute *attr, char *buf);

static ssize_t rmidev_sysfs_pid_show(struct rmidev_handle *rmidev,
		struct rmidev_attribute *attr, char *buf);

static ssize_t rmidev_sysfs_pid_store(struct rmidev_handle *rmidev,
		struct rmidev_attribute *attr, const char *buf, size_t count);

static ssize_t rmidev_sysfs_term_store(struct rmidev_handle *rmidev,
		struct rmidev_attribute *attr, const char *buf, size_t count);

static ssize_t rmidev_sysfs_intr_mask_show(struct rmidev_handle *rmidev,
		struct rmidev_attribute *attr, char *buf);

static ssize_t rmidev_sysfs_intr_mask_store(struct rmidev_handle *rmidev,
		struct rmidev_attribute *attr, const char *buf, size_t count);
...
static struct rmidev_attribute attrs[] = {
...
static loff_t rmidev_llseek(struct file *filp, loff_t off, int whence)
{
	...
	struct synaptics_rmi4_data *rmi4_data = dev_data->rmi_dev->rmi4_data;
		...
		size_t count, loff_t *f_pos)
{
	...
	retval = synaptics_rmi4_reg_read(dev_data->rmi_dev->rmi4_data,
			*f_pos,
			tmpbuf,
			count);
	if (retval < 0)
		...
		size_t count, loff_t *f_pos)
{
	...
	retval = synaptics_rmi4_reg_write(dev_data->rmi_dev->rmi4_data,
			*f_pos,
			tmpbuf,
			count);
	if (retval >= 0)
...
static int rmidev_create_device_class(void)
{
	if (!rmidev_device_class) {
		rmidev_device_class = class_create(THIS_MODULE,
				DEVICE_CLASS_NAME);
...
static int rmidev_attn(struct synaptics_rmi4_data *rmi4_data,
		unsigned char intr_mask)
{
	struct rmidev_handle *rmidev = rmi4_data->rmidev;
...
static ssize_t rmidev_attr_show(struct kobject *kobj, struct attribute *attr,
			     char *buf)
{
	struct rmidev_attribute *rmiattr = container_of(attr,
			struct rmidev_attribute, attr);
	struct rmidev_handle *rmidev = container_of(kobj,
			struct rmidev_handle, sysfs_dir);
	ssize_t ret = -EIO;

	dev_dbg(rmidev->rmi4_data->pdev->dev.parent,
			"%s: %s\n", __func__, rmiattr->attr.name);

	if (rmiattr->show)
		ret = rmiattr->show(rmidev, rmiattr, buf);

	if (ret >= (ssize_t)PAGE_SIZE) {
		dev_err(rmidev->rmi4_data->pdev->dev.parent,
				"rmidev_attr_show: %s returned bad count\n",
				rmiattr->attr.name);
	}
...
static ssize_t rmidev_attr_store(struct kobject *kobj, struct attribute *attr,
			      const char *buf, size_t count)
{
	...
	if (rmiattr->store)
		ret = rmiattr->store(rmidev, rmiattr, buf, count);

	return ret;
}

static const struct sysfs_ops dev_sysfs_ops = {
	.show	= rmidev_attr_show,
	.store	= rmidev_attr_store,
};

static struct kobj_type rmidev_ktype = {
...
static int rmidev_init_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	kobject_init(&rmidev->sysfs_dir, &rmidev_ktype);
	retval = kobject_add(&rmidev->sysfs_dir,
			&rmi4_data->input_dev->dev.kobj, SYSFS_FOLDER_NAME);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
...
int synaptics_rmidev_module_init(struct synaptics_rmi4_data *rmi4_data)
{
	synaptics_rmi4_new_function(rmi4_data, &rmidev_module, true);
